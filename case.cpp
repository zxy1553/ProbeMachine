...
// 计算过程
std::vector<std::vector<tuple<string, string, string>>> pft;
    ...
    while (cnt < n) // 没有新增聚合体，说明探针过程完成
    {
        for (int i = 0; i < pf.size(); i++)
        {
            for (int j = 0; j < pf.size(); j++)
            {
                ...
                {
                    pft[cnt].push_back(make_tuple(get<0>(pf[i]) + get<0>(pf[j]), get<1>(pf[i]), get<2>(pf[j])));
                }
            }
        }
        // 大的聚合体放前面
        pf.insert(pf.end(), pft[cnt].begin(), pft[cnt].end());
    ...
    for (size_t i = 0; i < canNum; i++)
    {
        for (size_t k = 0; k < 3; k++)
        {
            if (vote[i][k] == 0)
            {
                ...
                std::remove(kripke.transitions.begin(), kripke.transitions.end(), (std::pair<std::string, std::string>)make_pair(to, to));
                // 删去点
...
int main()
{
    ...
    MoM mom = genMoM(voteNum, canNum);
    Kripke kripke = genKripke(mom, canNum);
    bool f = EFModelChecking(mom, kripke, canNum, "v1", agNum);
    auto end = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
    double seconds = duration.count() / 1000000.0;

    // 输出程序运行结果
    std::cout << "************* Data Library X ****************" << endl;
    int cnt = 0;
    for (auto &d : dataLib)
    {
        cout << std::get<0>(d) << "-" << get<1>(d) << "-" << get<2>(d) << "\t";
        if (++cnt == 5)
        {
            cout << endl;
            cnt = 0;
    ...
    {
        std::cout << "**************** Step" + to_string(i + 1) + " ********************" << endl;
        auto tfp = pft[i];
        cnt = 0;
        sort(tfp.begin(), tfp.end(), cmp);
        for (auto &p : tfp)
        {
            cout << std::get<0>(p) << "-" << std::get<1>(p) << "-" << std::get<2>(p) << "\t";
    ...
    cout << "********************************" << endl;
    std::cout << "voters:    " << voteNum << endl
              << "candidates:" << canNum << endl
              << "agents:    " << agNum << endl;
    std::cout << "**************** Result ****************" << endl;
    std::cout << (f ? "True" : "False") << endl;
    cout << "Path is: " << std::get<0>(pf[0]) << endl;
